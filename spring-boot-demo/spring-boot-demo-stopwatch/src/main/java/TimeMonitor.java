...
public class TimeMonitor {
     ...
     * 默认是纳秒
     */
    public static TimeTypeEnum printTimeType = TimeTypeEnum.NANOS_TYPE;
     ...
     * 第一个任务开始时间
     */
    private long firstTaskBeginTimeNanos;
    /**
     * 最后一个任务结束时间
     */
    private long lastTaskEndTimeNanos;
    /**
     * 打印的日期格式
     */
    private DateFormat printDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
    ...
    public void start(String taskName) throws IllegalStateException {
        ...
        if (taskCount == 0) {
            this.firstTaskBeginTimeNanos = System.currentTimeMillis();
        }
    }
    ...
    public void stop() throws IllegalStateException {
        ...
        this.lastTaskEndTimeNanos = System.currentTimeMillis();
    }
     ...
     */
    public StringBuilder shortSummary() {
        StringBuilder sb = new StringBuilder();
        sb.append("TimeMonitor ‘").append(getId()).append("'")
                .append(": start time=").append(printDateFormat.format(new Date(firstTaskBeginTimeNanos)))
                .append("\n---------------------------------------------\n")
                .append("running time=").append(getTotalTime()).append(" ").append(printTimeType.name);
        return sb;
    }
    ...
    public Object getTotalTime() {
        switch (printTimeType) {
            case NANOS_TYPE:
                return getTotalTimeNanos();
            case MILLISECONDS_TYPE:
    ...
    public void prettyPrint() {
        ...
        StringBuilder sb = shortSummary();
        sb.append('\n');
        if (!this.keepTaskList) {
            sb.append("No task info kept");
        } else {
            sb.append("---------------------------------------------\n");
            sb.append(printTimeType.name).append("         %     Task name\n");
            sb.append("---------------------------------------------\n");
            ...
            for (TaskInfo task : getTaskInfo()) {
                sb.append(nf.format(task.getTime())).append("  ");
    ...
    public void averagePrint() {
        ...
        if (!this.keepTaskList) {
            ...
            sb.append("---------------------------------------------\n");
            sb.append("TaskCount:").append(taskCount).append("     ").append("AverageTime:").append(getAverageTime()).append(" ").append(printTimeType.name);
        }
        System.out.println(sb.toString());
    }
    ...
    public Object getAverageTime(){
        switch (printTimeType) {
            case NANOS_TYPE:
                return getAverageTimeNanos();
            case MILLISECONDS_TYPE:
                return getAverageTimeMills();
            case SECONDS_TYPE:
                return getAverageTimeSeconds();
            default:
                //NANOS_TYPE是默认值
                return "";
        }
    ...
    public long getAverageTimeNanos() {
        return getTotalTimeNanos() / taskCount;
    }

    public long getAverageTimeMills() {
        return getTotalTimeMillis() / taskCount;
    }

    public double getAverageTimeSeconds() {
        return getTotalTimeSeconds() / taskCount;
    }
