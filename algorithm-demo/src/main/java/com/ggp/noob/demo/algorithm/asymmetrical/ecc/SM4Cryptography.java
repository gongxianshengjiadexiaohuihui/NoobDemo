 ...
 * 混淆原则就是将密文、明文、密钥三者之间的统计关系和代数关系变得尽可能复杂，使得敌手即使获得了密文和明文，也无法求出密钥的任何信息;
 * 即使获得了密文和明文的统计规律，也无法求出明文的任何信息。(通过线性变换L和非线性变换t)
 *
 * 扩散原则就是应将明文的统计规律和结构规律散射到相当长的一段统计中去。
 * 也就是说让明文中的每一位影响密文中的尽可能多的位，或者说让密文中的每一位都受到明文中的尽可能多位的影响。(通过轮函数和轮秘钥)
 */
